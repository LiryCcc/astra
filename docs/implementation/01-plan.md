...
1. 欢迎区文案（本地化）
2. 接入 `uncompletedCount` 与总数统计
3. 快捷入口跳转 `/todos`、`/settings`
4. 可选：展示当前主题与语言
5. 响应式卡片布局（手机 1 列，平板/桌面 2 列）

**完成标志**：首页数据与 Todo 同步，入口可跳转。
...
P3（Todo）与 P4（首页）均依赖 P2；P4 的统计依赖 P3 的 Todo 数据，因此实际顺序为 P3 → P4。
